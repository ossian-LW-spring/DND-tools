...
import { 
  ...
  Smile, Train, Home, Undo, Redo, Check, Edit3
} from 'lucide-react';
...
export default function App() {
  ...
  // -- Auto Deselect when Tool Changes --
  useEffect(() => {
  ...
  const applyTool = (q: number, r: number, isDrag = false, modifiers: { shiftKey: boolean, ctrlKey: boolean, metaKey: boolean } = { shiftKey: false, ctrlKey: false, metaKey: false }) => {
    ...
    if (activeTool === TOOLS.PAINT) {
      ...
      const isRoadMode = ['paved', 'trail', 'rail', 'bulldoze_road'].includes(activeTerrain);
  ...
  const renderGrid = () => {
    ...
    Object.keys(gridData).forEach(key => {
      ...
      const showRegionHighlight = activeTool === TOOLS.SELECT && hasRegion;
